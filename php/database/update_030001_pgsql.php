...
$result = $database->exec("ALTER TABLE ".LYCHEE_TABLE_ALBUMS." ALTER title TYPE CHARACTER VARYING(100)");
if ($result === FALSE) {
	Log::error($database, 'update_030001', __LINE__, 'Could not update database (' . print_r($database->errorInfo(), TRUE) . ')');
	return false;
}
$result = $database->exec("ALTER TABLE ".LYCHEE_TABLE_ALBUMS." ALTER title SET DEFAULT ''");
if ($result === FALSE) {
	Log::error($database, 'update_030001', __LINE__, 'Could not update database (' . print_r($database->errorInfo(), TRUE) . ')');
	return false;
}
